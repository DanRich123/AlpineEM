module circuit_mod
    ...
    type VI_t
        real(kind=c_double) :: voltage
        real(kind=c_double) :: current
...
contains
    ...
    subroutine init(this, names, sources, netlist)
        ...
        ! Fix: Initialize the node names and their lengths
        do i = 1, size(names)
            this%nodes%names(i)%name = names(i)%name
            this%nodes%names(i)%length = len_trim(names(i)%name)
        end do
    end subroutine
   ...
   subroutine loadNetlist(this, netlist)
        ...
        character(len=256, kind=c_char) :: cmd_string
        ! Create a null-terminated C string for the command
        cmd_string = "source "//trim(netlist)//c_null_char
        call command(cmd_string)
    end subroutine
    ...
    subroutine updateNodes(this) 
        ...
        type(c_ptr) :: info_c_ptr
        real(kind=c_double), pointer :: values(:)
        character(len=256) :: node_name_c_string
        do i = 1, size(this%nodes%names)
            ! Fix: Handle 'time' and 'V(node)' separately
            if (trim(this%nodes%names(i)%name) == "time") then
                node_name_c_string = trim(this%nodes%names(i)%name)//c_null_char
            else
                node_name_c_string = 'V(' // trim(this%nodes%names(i)%name) // ')'//c_null_char
            end if
            
            ! Fix: Store the result of get_vector_info in a c_ptr variable before using it in c_f_pointer
            info_c_ptr = get_vector_info(node_name_c_string)

            ! Fix: Associate the Fortran pointer 'info' with the C pointer
            call c_f_pointer(info_c_ptr, info)

            ! Fix: The shape argument of c_f_pointer must be an integer array.
    ...
    function getTime(this) result(time)
        ...
        ! Get a C pointer to the 'time' vector info
        info_c_ptr = get_vector_info("time" // c_null_char)

        ! Associate the Fortran pointer with the C pointer
        call c_f_pointer(info_c_ptr, info)
        
        ! Associate the Fortran array pointer with the C data
        call c_f_pointer(info%vRealData, values, shape=[info%vLength])

        ! Return the last value from the time vector
        time = values(ubound(values, 1))
