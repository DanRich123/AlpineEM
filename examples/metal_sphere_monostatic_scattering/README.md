...
This example is designed to validate the core mechanisms of the solver including the total field / scattered field formalism (TF/SF), the convolutional perfecly matched layer (CPML) boundaries, the Near-to-Far-Field transformation (NTFF), and the three builder options (single-threaded CPU, multi-threaded CPU (OpenMP), and the GPU (OpenACC)).
